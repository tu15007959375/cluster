...
int call_anonymouslib(int m, int n, int nnzA,
                  ...
                  VALUE_TYPE *x, VALUE_TYPE *y, VALUE_TYPE alpha, int device)
{
    int err = 0;
    ...
    if (NUM_RUN)
            cout <<gflop/(1.0e+6 * CSR5Spmv_time)<<" ";
...
int main(int argc, char ** argv)
{
    ...
    //！ cout << "cpu sequential time(朴素计算) = " << ref_time
    //      << " ms. Bandwidth = " << gb/(1.0e+6 * ref_time)
    //      << " GB/s. GFlops = " << gflop/(1.0e+6 * ref_time)  << " GFlops." << endl ;

    // launch compute
    call_anonymouslib(m, n, nnzA, csrRowPtrA, csrColIdxA, csrValA, x, y, alpha,device);
